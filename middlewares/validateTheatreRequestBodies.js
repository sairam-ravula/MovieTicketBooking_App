  ...
  try {
    ...
    if (!req.body.showTypes) {
      ...
      if (!Array.isArray(req.body.showTypes)) {
        return res.status(400).send({
          message:
            "Failed ! Movie show types are not in correct format (Array)",
        });
    ...
    console.log(
      "#### Error while velidating new theatre request body ##### ",
      err.message
    );
    res.status(500).send({
...
const editTheatreBody = (req, res, next) => {
  try {
    ...
    console.log(
      "#### Error while velidating edit theatre request body ##### ",
      err.message
    );
    res.status(500).send({
...
const editMoviesInTheatreBody = async (req, res, next) => {
  try {
    ...
    console.log(
      "#### Error while velidating edit movie in theatre request body ##### ",
      err.message
    );
    res.status(500).send({
      message:

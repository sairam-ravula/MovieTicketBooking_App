...
const validateBookingBody = async (req, res, next) => {
  //this validation itself is used to validate the
  ...
  try {
    ...
    if (req.body.noOfSeats) {
      if (typeof req.body.noOfSeats !== "number") {
        ...
        req.body.noOfSeats > theatre.numberOfSeats ||
...
const validateUpdateBookingReqBody = async (req, res, next) => {
  try {
    if (req.body.bookingStatus) {
      ...
      if (req.user.userType == constants.userTypes.customer) {
        if (req.body.bookingStatus == constants.bookingStatuses.cancelled) {
          if (
            req.bookingInParams.bookingStatus ==
            ...
            return res.status(400).send({
              ...
              message:
                "You can't change the status of the booking which is alraedy failed",
            });
          }
        } else {
    ...
    console.log(
      "error while validating the update booking req body",
      err.message
    );
